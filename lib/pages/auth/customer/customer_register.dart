...
import 'package:delivery_app/model/response/customer_register_post_res.dart';
...
class _CustomerRegisterState extends State<CustomerRegister> {
  ...
  Future<void> Submit() async {
    final url = Uri.parse("$API_ENDPOINT/users/register");
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final resData = customerRegisterPostResponseFromJson(responseBody);
        showDialog(
          context: context,
          builder: (context) => AlertDialog(
            title: const Text("สมัครสมาชิกสำเร็จ"),
            content: Text(
              "ยินดีต้อนรับ ${resData.user.name}\nเบอร์โทร: ${resData.user.phone}",
            ),
            actions: [
              TextButton(
        ...
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text(data['error'] ?? "เกิดข้อผิดพลาด"),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context),
                child: const Text("ตกลง"),
